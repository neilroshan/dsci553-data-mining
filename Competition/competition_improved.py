...
def getOutdoorSeating(x):
    try:
        if 'OutdoorSeating' in x.keys():
            return returnBool(x['OutdoorSeating'])
        else:
            return 0.0
    except:
        return 0.0
...
def getDataset(folderPath,chunkSize,case):
    ...
    for chunk in chunkIterator:
        if case=='user':
            chunk_filter = chunk[['user_id','average_stars','review_count','useful','fans', 'compliment_note', 'compliment_hot']]
        else:
            chunk_filter = chunk[['business_id','stars','review_count','is_open','attributes']]
