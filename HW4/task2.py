...
def GirvanNewman(user_graph):
    ...
    def calculate_betweeness(root):
        ...
        for i in range(1,level+1):
            for child in levelOrder[i]:
                ...
                for parent in parents:
                    no_of_paths += shortest_paths[parent]
        ...
        betweenness = [(tuple(sorted(k)),v) for k,v in betweenness_dict.items()]
        return betweenness
...
def DFS_helper(temp, v, vertices, visited, graph):
    visited[v] = True
    temp.append(v)

    for i in graph[v]:
        if visited[i] == False:
            temp = DFS_helper(temp, i, vertices, visited, graph)
    return temp

def getConnectedComponents(vertices, graph):
    visited = dict()
    components = []
    for v in vertices:
        visited[v] = False
    ...
    for v in vertices:
        if visited[v]==False:
            temp = []
            components.append(DFS_helper(temp, v, vertices, visited, graph))
    return components

def calculateModularity(components, A_matrix, k_dict):
    modulariy_list = []
    for commmunity in components:
        for user1 in commmunity:
            for user2 in commmunity:
                if user1 in A_matrix[user2] or user2 in A_matrix[user1]:
                    A_ij = 1
                else:
                    A_ij = 0
                k_i, k_j = k_dict[user1],k_dict[user2]
                # print(A_ij)
                # print(((k_i*k_j)/2*m))
                modularity = A_ij - ((k_i*k_j)/(2*m))
                modulariy_list.append(modularity)
    return sum(modulariy_list)/(2*m)
...
if __name__=='__main__':
    ...
    A_matrix = copy.deepcopy(user_graph)
    ...
    m = noOfedges/2
    #print(m)
    true_noOfedges = noOfedges/2

    k_dict = dict()

    for user1,user2 in edges:
        k_dict[user1] = len(A_matrix[user1])
        k_dict[user2] = len(A_matrix[user2])
    #print(k_dict)

    while true_noOfedges!=0:

        highest_betweeness = betweeness_list[0][1]
        for i in betweeness_list:
            key, betweeness = i
            if betweeness == highest_betweeness:
                user_graph[key[0]].remove(key[1])
                user_graph[key[1]].remove(key[0])
                betweeness_list.remove(i)
                true_noOfedges -= 1
            else:
                continue

        components = getConnectedComponents(vertices, user_graph)
        new_modularity = calculateModularity(components, A_matrix, k_dict)
    ...
    sorted_communities = sc.parallelize(current_components).map(lambda x: sorted(x)).sortBy(lambda x: (len(x), x)).collect()
